...
export default function PseudoLegendaryPokemonPage() {
  const { t } = useLanguage()
  ...
  const category = pokemonCategories.pseudoLegendary
  const categoryPokemon = useMemo(() => getAllCategoryPokemon('pseudoLegendary'), [])
    ...
    <section className="space-y-6">
      ...
      {paginatedData.length === 0 ? (
        ...
        <>
          ...
          {selectedGeneration !== 'all' && category.subcategories && (
            <section className="bg-purple-50 p-4 rounded-lg border border-purple-200">
              <h3 className="text-lg font-semibold mb-3 text-purple-900">Evolution Chain</h3>
              <div className="flex flex-wrap items-center gap-2">
                {category.subcategories
                  .find(sub => sub.name === selectedGeneration)
                  ?.pokemon.map((pokemonName, index, array) => (
